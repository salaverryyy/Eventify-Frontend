...
export const getMemoryDetails = async (memoryId: number) => {
  ...
  const response = await axios.get(`http://localhost:8080/memories/${memoryId}`, {
    headers: {
      Authorization: `Bearer ${localStorage.getItem('token')}`,
    },
  });
...
export const fetchAlbumDetails = async (memoryId: string) => {
  ...
  if (isUUID) {
    ...
    const response = await axios.get(`http://localhost:8080/memories/${memoryId}/album-uuid`, {
      headers: {
        Authorization: `Bearer ${localStorage.getItem('token')}`,
      },
    });
...
export const searchUsersByUsername = async (username: string) => {
  const response = await axios.get(`http://localhost:8080/usuarios/search?username=${username}`, {
    headers: {
      Authorization: `Bearer ${localStorage.getItem('token')}`,
    },
  });
...
export const createPublication = async (
  ...
  const response = await axios.post(
    `http://localhost:8080/publication/${localStorage.getItem('selectedMemoryId')}`,
    formData,
    {
      headers: {
        Authorization: `Bearer ${localStorage.getItem('token')}`,
